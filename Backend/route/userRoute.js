...
import { userModel, propertyModel, buyerModel, contactUsModel } from '../model/table.js';
...
router.post("/add-contact-us" ,async(req,res)=>{
  ...
  console.log(message);
  const data=new contactUsModel({name,email,contact,subject,message})
  const result=await data.save();
  if(result){
    res.json({
        code: 200,
...
router.get("/get-contact-us" ,async(req,res)=>{
  // const{name,email,contact,subject,statement}=req.body
  // const data=new contactUsModel({name,email,contact,subject,statement})
